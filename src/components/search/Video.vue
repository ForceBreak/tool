    ...
    document.addEventListener("mousemove", event => {
      if(focus && mouseDown){
        if(focus.className.includes("plane")){
          left = event.clientX - focus.offsetWidth / 2
          top = event.clientY - focus.offsetHeight / 2
          if(left >= 0 && ((left + focus.offsetWidth) <= this.dragAreaWidth)) focus.style.left = `${left}px`;
          if(top >= this.dragAreaFromTop) focus.style.top = `${top}px`;
        } else {
